...
struct PolygonFace
{
	...
	PolygonFace(bool _guranteeConvex) : GuaranteeConvex(_guranteeConvex), FacePlaneConstructed(false), ForceColor(false) {}
	PolygonFace(bool _guranteeConvex, std::vector<Vector3> _vertexVec)
		: GuaranteeConvex(_guranteeConvex), VertexVec(_vertexVec), ForceColor(false)
	{
		ConstructFacePlane();
	}
	...
	static PolygonFace ClipWithPlane(const PolygonFace& inFace, const Plane& clippingPlane,
									 std::vector<PolygonEdge>& edgeVec);
	static PolygonFace ClipWithFace(const PolygonFace& inFace, const PolygonFace& clippingFace,
									std::vector<PolygonEdge>& edgeVec);
};
...
struct Polygon3D
{
	...
	Polygon3D(bool _guranteeConvex, std::vector<PolygonFace> _faceVec)
		: GuaranteeConvex(_guranteeConvex), FaceVec(_faceVec)
	{
	}
	...
	static Polygon3D ClipWithPlane(const Polygon3D& inPolygon, const Plane& clippingPlane, int doTest = -1);
...
private:
	...
	void CreateFace(const ConvexHullVertex& p1, const ConvexHullVertex& p2, const ConvexHullVertex& p3,
...
struct VisualMesh
{
	...
	void Init(const std::vector<VertexNormalColor>& vertices, const std::vector<uint32_t>& indices);
	void Scale(const Vector3& vector);
	void Translate(const Vector3& vector);

	void       Render(std::vector<VertexNormalColor>& vertexData, std::vector<uint32_t>& indexData,
					  DirectX::SimpleMath::Vector3 color = { 0.25f, 0.25f, 0.25f });
	VisualMesh Clipping(const DirectX::SimpleMath::Plane& clippingPlane) const;
};
...
double    CalcDistanceToPoint(const Vector3& point, const Plane& plane);
bool      GetIntersectionPoint(const Vector3& p1, const Vector3& p2, const Plane& plane, Vector3& intersection);
